...
class SimpleTips {
  opts: SimpleTipsOpts
  tipsEl: HTMLElement | null
  tipsTimer: number | null

  constructor(opts?: SimpleTipsOpts) {
    const defOpts = {
      fontSize: 16,
      parentNode: null,
  ...
  create() {
    ...
    tips.setAttribute('class', opts.tipsClassName as string)
    parentNode.appendChild(tips)
    t.tipsEl = tips
  }

  tips(msg: string, parentNode?: HTMLElement) {
    const t = this
  ...
  _show() {
    ...
    if (tipsEl && tipsEl.style) {
      clearTimeout(this.tipsTimer as number)
